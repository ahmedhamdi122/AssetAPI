    ...
    public class MasterAssetRepositories : IMasterAssetRepository
    {
        ...
        public IEnumerable<string> GetDistintMasterAssetModels(int brandId, string name)
        {
            ...
            var filteredAssets = _context.MasterAssets
                .Where(a => a.Name == name  || a.NameAr == name && a.BrandId == brandId)
                .ToList();

            // Group the assets by Brand
            var groupedByBrand = filteredAssets
               // .GroupBy(a =>new { a.Name,a.NameAr,a.BrandId })
                 .GroupBy(a =>   a.BrandId )
                .Select(group => new
                {
                    //Name = group.Key,
                    //NameAr = group.Key,
                    BrandId = group.Key,   // The brand name
                    Assets = group.ToList() // List of assets under that brand
                })
                .ToList();

            // Check if there are any brands in the result
            if (groupedByBrand.Count > 0)
            {
                // If you want to return the brand list (only the brand names), you can use:
                //var brandList = groupedByBrand.Select(g => g.ModelNumber).ToList();

                var brandList = groupedByBrand.SelectMany(g => g.Assets).Select(a => a.ModelNumber).ToList();
                return brandList;
            }
            return null;
